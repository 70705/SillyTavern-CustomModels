...
for (const [provider, models] of Object.entries(settings.provider)) {
  const sel = document.querySelector(`#model_${provider}_select`);
  if (!sel) continue;
  const h4 = sel.previousElementSibling;
  if (!h4 || h4.tagName !== "H4") continue;
  if (provider === "vertexai") {
    h4.textContent = "Vertex AI Model";
  }
    ...
    btn.addEventListener("click", async () => {
      ...
      if (result == popupResult.AFFIRMATIVE) {
        ...
        if (
          ...
          models.includes(settings[`${provider}_model`])
        ) {
